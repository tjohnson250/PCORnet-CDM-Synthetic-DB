...
# Create databases
con_cdw <- dbConnect(RSQLite::SQLite(), ":memory:")
con_mpi <- dbConnect(RSQLite::SQLite(), ":memory:")
...
# Convert dates to character strings for proper SQLite storage
enterprise_records_db <- enterprise_records
enterprise_records_db$DoB <- as.character(enterprise_records_db$DoB)
enterprise_records_db$LastEditDTTM <- as.character(enterprise_records_db$LastEditDTTM)

dbWriteTable(con_mpi, "EnterpriseRecords", enterprise_records_db, overwrite = TRUE)
...
# Convert dates to character strings for proper SQLite storage
enterprise_ext_db <- enterprise_ext
enterprise_ext_db$MPIUpdateDTTM <- as.character(enterprise_ext_db$MPIUpdateDTTM)
enterprise_ext_db$DeceasedDTTM <- as.character(enterprise_ext_db$DeceasedDTTM)

dbWriteTable(con_mpi, "EnterpriseRecords_Ext", enterprise_ext_db, overwrite = TRUE)
...
# Convert dates to character strings for proper SQLite storage
demographic_db <- demographic
demographic_db$BIRTH_DATE <- as.character(demographic_db$BIRTH_DATE)
demographic_db$CDW_UpdatedDTTM <- as.character(demographic_db$CDW_UpdatedDTTM)

dbWriteTable(con_cdw, "DEMOGRAPHIC", demographic_db, overwrite = TRUE)
...
death_records <- enterprise_ext %>%
  ...
  transmute(
    ...
    DEATH_DATE = as.character(as.Date(DeceasedDTTM)),
    DEATH_DATE_IMPUTE = "N",
    DEATH_SOURCE = sample(c("L", "N", "S"), n(), replace = TRUE),
    DEATH_MATCH_CONFIDENCE = NA,
    GPC_FLAG = "Y",
    CDW_UpdatedDTTM = as.character(CURRENT_DATETIME),
    UID = Uid,
    MPI_LID = NA,
    MPI_SRC = NA
  )
...
encounter_db$ADMIT_DATE <- as.character(encounter_db$ADMIT_DATE)
encounter_db$DISCHARGE_DATE <- as.character(encounter_db$DISCHARGE_DATE)
encounter_db$CDW_UpdatedDTTM <- as.character(encounter_db$CDW_UpdatedDTTM)

dbWriteTable(con_cdw, "ENCOUNTER", encounter_db, overwrite = TRUE)
...
diagnosis_db$ADMIT_DATE <- as.character(diagnosis_db$ADMIT_DATE)
diagnosis_db$DX_DATE <- as.character(diagnosis_db$DX_DATE)
diagnosis_db$CDW_UpdatedDTTM <- as.character(diagnosis_db$CDW_UpdatedDTTM)

dbWriteTable(con_cdw, "DIAGNOSIS", diagnosis_db, overwrite = TRUE)
...
if (length(procedures) > 0) {
  ...
  # Convert dates to character strings for proper SQLite storage
  procedure_db <- procedure
  procedure_db$ADMIT_DATE <- as.character(procedure_db$ADMIT_DATE)
  procedure_db$PX_DATE <- as.character(procedure_db$PX_DATE)
  procedure_db$CDW_UpdatedDTTM <- as.character(procedure_db$CDW_UpdatedDTTM)

  dbWriteTable(con_cdw, "PROCEDURES", procedure_db, overwrite = TRUE)
...
if (length(lab_results) > 0) {
  ...
  # Convert dates to character strings for proper SQLite storage
  lab_result_cm_db <- lab_result_cm
  lab_result_cm_db$RESULT_DATE <- as.character(lab_result_cm_db$RESULT_DATE)
  lab_result_cm_db$LAB_ORDER_DATE <- as.character(lab_result_cm_db$LAB_ORDER_DATE)
  lab_result_cm_db$SPECIMEN_DATE <- as.character(lab_result_cm_db$SPECIMEN_DATE)
  lab_result_cm_db$CDW_UpdatedDTTM <- as.character(lab_result_cm_db$CDW_UpdatedDTTM)

  dbWriteTable(con_cdw, "LAB_RESULT_CM", lab_result_cm_db, overwrite = TRUE)
  ...
  if (n_rx > 0) {
    for (j in 1:n_rx) {
      ...
      prescriptions[[rx_id]] <- data.frame(
        ...
        RX_DOSE_ORDERED = medications$DOSE[med_idx],
        RX_DOSE_ORDERED_UNIT = medications$UNIT[med_idx],
        RX_FREQUENCY = sample(c("01", "02", "03"), 1),
...
if (length(prescriptions) > 0) {
  ...
  # Convert dates to character strings for proper SQLite storage
  prescribing_db <- prescribing
  prescribing_db$RX_START_DATE <- as.character(prescribing_db$RX_START_DATE)
  prescribing_db$RX_END_DATE <- as.character(prescribing_db$RX_END_DATE)
  prescribing_db$CDW_UpdatedDTTM <- as.character(prescribing_db$CDW_UpdatedDTTM)

  dbWriteTable(con_cdw, "PRESCRIBING", prescribing_db, overwrite = TRUE)
...
for (i in 1:nrow(encounter)) {
  ...
  if (runif(1) < 0.9) {
    ...
    vitals[[vital_id]] <- data.frame(
      ...
      HT = round(rnorm(1, 170, 15), 1),  # cm
      WT = round(rnorm(1, 80, 20), 1),   # kg
    ...
    # Calculate BMI
    vitals[[vital_id]]$ORIGINAL_BMI <- round(
      vitals[[vital_id]]$WT / (vitals[[vital_id]]$HT/100)^2, 1
    )
...
if (length(vitals) > 0) {
  ...
  # Convert dates to character strings for proper SQLite storage
  vital_db <- vital
  vital_db$MEASURE_DATE <- as.character(vital_db$MEASURE_DATE)
  vital_db$CDW_UpdatedDTTM <- as.character(vital_db$CDW_UpdatedDTTM)

  dbWriteTable(con_cdw, "VITAL", vital_db, overwrite = TRUE)
...
providers <- data.frame(
  ...
  PROVIDER_NPI = as.numeric(sprintf("1%09d", sample(100000000:999999999, 500))),
  GPC_FLAG = "Y",
  stringsAsFactors = FALSE
)
...
con_cdw_disk <- dbConnect(RSQLite::SQLite(), "pcornet_cdw.db")
con_mpi_disk <- dbConnect(RSQLite::SQLite(), "mpi.db")
...
dbDisconnect(con_cdw_disk)
dbDisconnect(con_mpi_disk)

cat("Databases saved:\n")
cat("  pcornet_cdw.db - Clinical Data Warehouse\n")
cat("  mpi.db - Master Patient Index\n")
